...
## D-015 · 舊文時效性：原地標註時間戳，不下架也不定期重寫

- **日期**：2026-08-08
- **背景**：`content/` 有一批 2026-05 從 HackMD／課堂講義搬上來的文章。逐篇查證後發現三類問題混在一起：**真正的過時**（Suno 停在 v3.5、器材停在 Pocket 3、Anchor 品牌已不存在、派拉蒙判決令 2020 就終止了）、**與時間無關的事實錯誤**（「Midjourney 基底是 Stable Diffusion」、「獨角獸是市值百億美元」差一個數量級）、以及**法律風險**（整首歌詞全文轉載、導流到違反 YouTube ToS 的下載站）
- **選項考慮過**：
  - 把會過時的文章下架 —— 內容本身仍有價值，讀者也還在看，下架是最浪費的一種處理
  - 定期全面重寫 —— 維護成本不可持續，個人知識庫撐不起這個節奏
  - 完全不管，讓讀者自己判斷 —— 錯誤資訊會持續誤導人，而且傷的是站的可信度
  - **原地修正 + 標註時間戳**（選這個）
- **決定** —— 五條處理原則：
  1. **會變的段落標時間戳**：開頭寫「截至 2026-08」，讓讀者自己判斷新舊，而不是假裝內容永遠成立
  2. **不編造數字**：查不到當下的台灣售價就標「2025 年參考價」並請讀者自行確認，絕不硬掰
  3. **來源衝突就照實寫**：Runway 的免費點數是否重置，各家報導說法不一 → 直接寫「說法不一，以官網為準」
  4. **上一代不等於廢鐵**：Pocket 3、Mic 2 明確註記仍然堪用甚至更划算。DJI Mic 3 取消 3.5mm 輸入這種事要寫成「你要不要外接」的取捨，不是「新的比較好」
  5. **失效的參考資料誠實註記**：`sunoaiwiki.com` 整站 DNS 失效（4 個連結全死），換成實測可開的替代來源，並直說原引用已失效
- **理由**：
  - 這個站的價值在**方法與原理**，那些不會過時；過時的是型號、版本號、價格這些外圍事實。標註時間戳能保住前者、同時對後者誠實
  - 「原地修正」比「下架」或「重寫」都便宜，而且**修正本身常常讓文章更好**——例如派拉蒙判決令被終止這件事，反而把該案的教學重點從「這種結構違法」提升到「反托辣斯判斷的是當下市場結構裡的競爭傷害」
- **版權尺度**（同時定版）：
  - **歌詞／文本只引用實際討論到的段落**，不做全文轉載。Space Bound 原本整首歌詞照貼，已改為只留分析用的副歌四句
  - **不導流到違反平台服務條款的工具**。YouTube 下載站不移除（讀者有知的權利）但**拿掉外連**、改成風險提醒，並補上合法替代路徑
- **查證紀律**（本次代價換來的）：
  - **「這個好像過時了」的直覺一律要驗證後才動手**。本次三次差點改錯：HuggingChat 2025-07 關閉後**又重新上線**、pyTranscriber 看似老工具但 2026-06 仍在更新（而且新版才讓原文「可離線」那句成真）、大量 403 是擋 bot 不是死站
  - 對照 D-010 的外部連結檢查：那條 CI 只驗「網址通不通」，**驗不出「產品還在不在」「版本對不對」**。後者目前沒有自動化手段，只能靠人工週期性體檢
- **未解**：
  - 沒有機制提醒「哪篇文章該回頭看了」。目前靠人工想到才查
  - `created` 之外沒有「內容有效期」的欄位，讀者只能從時間戳的文字敘述判斷
  - 器材那篇的 GoPro 世代未逐一查證，已改為泛稱「GoPro HERO 系列（型號逐年更新）」迴避
